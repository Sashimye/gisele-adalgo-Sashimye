...
test("il/elle are properly replaced", () => {
    ...
    expect(
        inclusify(
            "Il a mangé tout les chocolats et elles ont été en racheter.",
        ),
...
test("ceux/celle are properly replaced", () => {
    expect(inclusify("Ceux qui n'ont pas de parapluie, ne sortez pas")).toBe(
        "Celleux qui n'ont pas de parapluie, ne sortez pas",
    );
    expect(inclusify("Ils ont mangé celle qui n'était pas piquant")).toBe(
        "Iels ont mangé celleux qui n'était pas piquant",
    );
});
